    ...
    while run:
        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.KEYDOWN:
                ...
                MAX_BULLETS = 3
                if event.key == pygame.K_SPACE and len(yellow_bullets) < MAX_BULLETS:  # Spacebar for Yellow
                    bullet = pygame.Rect(yellow.x + yellow.width, yellow.y + yellow.height // 2 - 2, 10, 5)
                    yellow_bullets.append(bullet)
            
                if event.key == pygame.K_RETURN and len(red_bullets) < MAX_BULLETS:  # Enter for Red
